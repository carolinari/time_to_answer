class AdminsBackoffice::SubjectsController < AdminsBackofficeController
    ...
    def index
      ...
      # Filtra os subjects com base na categoria (description)
      if params[:category].present?
        @subjects = @subjects.where(description: params[:category])
      end
      # Para filtrar também pela pesquisa de termo (se houver)
      if params[:term].present?
        @subjects = @subjects.where("description LIKE ?", "%#{params[:term]}%")
      end
    end
